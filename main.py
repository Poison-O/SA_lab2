...
form_class, base_class = loadUiType("main_window_mo.ui")
...
class MainWindow(QDialog, form_class):
    ...
    def input_clicked(self):
        filename = QFileDialog.getOpenFileName(
            self, "Open data file", ".", "Data file (*.txt *.dat)"
        )[0]
        if filename == "":
            return
    ...
    def output_clicked(self):
        filename = QFileDialog.getSaveFileName(
            self, "Save data file", ".", "Spreadsheet (*.xlsx)"
        )[0]
        if filename == "":
            return
    ...
    def exec_clicked(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Error!", "Error happened during execution: " + str(e)
            )
    ...
    @pyqtSlot(bool)
    def weights_modified(self, isdown):
        if isdown:
            sender = self.sender().objectName()
            if sender == "radio_avg":
                self.weight_method = "average"
            elif sender == "radio_norm":
                self.weight_method = "scaled"
        return
...
form = MainWindow()
form.setWindowTitle("Vyshemirskyi, Dolgopolova, Tovkach and Strashuk, variant 5")
form.show()
sys.exit(app.exec_())
