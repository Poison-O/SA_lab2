...
class Solve(object):
    ...
    def define_norm_vectors(self):
        ...
        self.X_ = [
            self.datas[:, : self.degf[0]],
            self.datas[:, self.degf[0] : self.degf[1]],
    ...
    def lamb(self):
        ...
        for i in range(self.deg[3]):
            if self.splitted_lambdas:
                ...
                lamb = np.append(
                    lamb, self._minimize_equation(self.A, self.B[:, i]), axis=1
                )
            ...
            for k in range(len(self.X)):  # choose X1 or X2 or X3
                for s in range(self.X[k].shape[1]):  # choose X11 or X12 or X13
                    for i in range(self.X[k].shape[0]):
                        psi[i, l] = (
                            self.A[i, q : q + self.p[k]] * lamb[q : q + self.p[k], 0]
                        )
                    q += self.p[k]
                    l += 1
    ...
    def built_a(self):
        ...
        for i in range(self.deg[3]):
            ...
            a2 = self._minimize_equation(
                self.Psi[i][:, self.degf[0] : self.degf[1]], self.Y[:, i]
            )
        ...
        for i in range(self.deg[3]):
            self.c = np.append(
                ...
                conjugate_gradient_method(
                    self.Fi[i].T * self.Fi[i], self.Fi[i].T * self.Y[:, i], self.eps
                ),
                axis=1,
            )
    ...
    def show(self):
        ...
        # polynoms = ["chebyshev", "legendre", "laguerre", "hermit"]
        # degrees = [
        #     [i, j, k] for i in range(1, 9) for j in range(1, 9) for k in range(1, 9)
        # ]
        # min_degrees = []
        # min_poly = ""
        # prev_min = 1
        ...
        #         if prev_min > sum(self.norm_error):
        #             prev_min = sum(self.norm_error)
        #             min_degrees = d
        #             min_poly = poly

        # print(min_poly, np.asarray(min_degrees)-1, prev_min)
        # # text.append(min_poly)
        # # text.append(tb(min_degrees))
        text.append("\nПохибка (нормалізована)")
